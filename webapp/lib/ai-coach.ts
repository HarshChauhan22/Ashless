// A deliberately simple, rule-based responder — NOT a call to a real LLM.
// Per the brief: "AI is a supporting layer, not the core financial engine,"
// and wiring a real model would need an API key/secret and explicit
// approval this prototype doesn't have. This keeps the Coach screen fully
// working for a demo (crisis-escalation guardrail included, since that's a
// safety requirement, not a nice-to-have) without connecting live external
// infrastructure. Swapping in a real Claude call later is a
// `respondTo()`-body-only change — the chat UI and message storage don't
// need to change at all.

const CRISIS_KEYWORDS = ["kill myself", "suicide", "end my life", "hurt myself", "self harm", "self-harm", "want to die", "no reason to live"];

// India helpline numbers per docs/architecture/system-architecture.md §9.2.
...
export function respondTo(userMessage: string, context: { intensity?: string | null; trigger?: string | null } = {}): string {
...
export function openingMessage(context: { intensity?: string | null; trigger?: string | null }): string {
  if (context.trigger) {
    return `Hey — I saw this one's tied to ${context.trigger.toLowerCase()}. Want to talk through it, or jump straight to something concrete?`;
  }
